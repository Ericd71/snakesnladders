...
int read_tile_line(Board* board, FILE* fd) {
    ...
    if (fscanf(fd, "%d%c%d", &position, &type, &target) == 3) {
        int checking = check_tile_data(position, type, target, size);
        if (checking == SUCCESS) {
            tile = get_tile_at(board, position);
            set_target_position(tile, target);
        }
        return checking;
    } else if (fscanf(fd, "%d%c%d", &position, &type, &target) != EOF){
...
int read_board_file(Board* board, FILE* fd) {
    ...
    int scanf = fscanf(fd, "%d%c%d", &rows, &x, &columns);

    if (scanf == 3 && rows > 0 && rows <= MAX_ROWS && columns > 0 && columns <= MAX_COLUMNS && x == 'x'){
        set_rows(board, rows);
