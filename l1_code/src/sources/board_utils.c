...
int check_tile_data(int position, char type, int target, int board_size) {
    if (position < 1 || position > board_size || (type != 'S' && type != 'L') || target < 1 || target > board_size)
...
int read_tile_line(Board* board, FILE* fd) {
    int position, target;
    char type; //ladder/snake
    if (fscanf(fd, "%d %c %d\n", &position, &type, &target) != 3){
        return INVALID_TILE_LINE;
    }

    int size = get_size(board);

    if (check_tile_data(position, type, target, size) == INVALID_TILE_DATA){
        return INVALID_TILE_DATA;
    }

    Tile* current_tile = get_tile_at(board, position);
    current_tile->position = position;
    current_tile->target = target;
    if (type == 'S'){
...
/**
 * TODO (2nd submission): Reads an opened board file, reading the dimensions line and each of the tile lines, configuring the board accordingly.
 * @param board The board to be configured.
 * @param fd The opened board file.
 * @return The status of the action:
 *      - SUCCESS if the file was loaded succesfully.
...
int read_board_file(Board* board, FILE* fd) {
    /*int rows, cols;
    if (fscanf(fd, "%d %d\n", &rows, &cols) != 2) return INVALID_BOARD_DIMENSIONS;
    if (init_board(board, rows, cols) == INVALID_BOARD_DIMENSIONS) return INVALID_BOARD_DIMENSIONS;

    int position, target;
    char type;

    while (fscanf(fd, "%d %c %d\n", &position, &type, &target) == 3) {
        if (check_tile_data(position, type, target, get_size(board)) == INVALID_TILE_DATA) {
            return INVALID_TILE_DATA;
        }

        int tile_index = position - 1;

        // Configure the tile with the given data.
        board->tiles[tile_index].target = target;

        // Set the tile type based on the character read from the file.
        if (type == 'S') {
            board->tiles[tile_index].type = SNAKE;
        } else if (type == 'L') {
            board->tiles[tile_index].type = LADDER;
        } else {
            return INVALID_TILE_DATA;
        }
    }
...
/**
 * TODO (2nd submission): Opens the file in path and tries to read a board configuration from it.
 *
 ...
 * @return
 *
 * Pre: board and path are not NULL.
 * Post: board contains the configuration read from the file, if successful.
 */
int load_board(Board* board, char* path) {
    ...
    for (int i = 0; i < board->rows; i++) {
        for (int j = 0; j < board->columns; j++) {
            // Read the tile value from the file
            int tile_value, tile_position;
            if (fscanf(file, "%d:%d", &tile_value, &tile_position) != 2) {
                fclose(file);
                return FALSE;
            }

            // Store the tile value in the board
            board->tiles[i][j].position = tile_value;
            board->tiles[i][j].target = (i * board->columns + j) + 1;
        }
    }
    fclose(file);
    return TRUE;
